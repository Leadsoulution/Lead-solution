...
const Dashboard: React.FC<DashboardProps> = ({ orders }) => {
  ...
  const ordersToday = orders.filter(o => new Date(o.date).toDateString() === today.toDateString()).length;
  const ordersThisWeek = orders.filter(o => new Date(o.date) >= startOfWeek).length;
  const ordersThisMonth = orders.filter(o => new Date(o.date) >= startOfMonth).length;

  const statusCounts = orders.reduce((acc, order) => {
    if (order.statut !== Statut.NonDefini) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatCard title="Total Revenue" value={formatCurrency(totalSales)} icon={<DollarSign className="h-4 w-4 text-muted-foreground" />} description="Total revenue from all sales" />
          ...
          <h2 className="text-lg font-semibold mb-4">Sales Overview</h2>
           <ResponsiveContainer width="100%" height={300}>
            <BarChart data={monthlySalesData}>
              ...
              <Tooltip cursor={{fill: 'hsla(var(--muted), 0.5)'}} contentStyle={{ backgroundColor: 'hsl(var(--background))', border: '1px solid hsl(var(--border))' }} formatter={(value: number) => [formatCurrency(value), "sales"]}/>
              <Legend />
